...
fn faceting_subdim(
    ...
    for orbit in &hyperplane_orbits {
        ...
        let (possible_facets_row, ff_counts_row, ridges_row, compound_facets_row) =
            faceting_subdim(rank-1, hp, points, new_stabilizer.clone(), min_edge_length, max_edge_length, max_per_hyperplane, include_compounds, exotic, uniform, None, false, false, false, false);
...
impl Concrete {
    ...
    pub fn faceting(
        ...
        for (idx, orbit) in hyperplane_orbits.iter().enumerate() {
            ...
            let (possible_facets_row, ff_counts_row, ridges_row, compound_facets_row) =
                faceting_subdim(rank-1, hp, points, new_stabilizer, min_edge_length, max_edge_length, max_per_hyperplane, include_compound_elements, exotic_elements, uniform, noble_package, true, hoshostaz, sidtid, rhom);
